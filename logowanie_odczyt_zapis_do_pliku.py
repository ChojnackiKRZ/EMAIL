...
for i in range (1, len (UIDs)+1):
    for j in UIDs:
        message = pyzmail.PyzMessage.factory(rawMessages[j] [b'BODY[]'])
    ...
    sheet["B"+str(i+1)]=nazwa_nadawcy
    sheet["C"+str(i+1)]=email_nadawcy
    sheet["D"+str(i+1)]=temat
    sheet["E"+str(i+1)]=tresc_tekst
    
wb.save('emaile_z_' + data2_zapisu +'.xlsx')
